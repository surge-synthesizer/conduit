...
cp ${INDIR}/License.txt "${TMPDIR}"
cp ${INDIR}/README.rtf "${TMPDIR}/Readme.rtf"
...
if [[ ! -z $MAC_INSTALLING_CERT ]]; then
  ...
  productbuild --sign "$MAC_INSTALLING_CERT" --distribution "distribution.xml" --package-path "." --resources "." "$OUTPUT_BASE_FILENAME.pkg"
else
  echo "Building UNSIGNED PKG"
  echo productbuild --distribution "distribution.xml" --package-path "."  "$OUTPUT_BASE_FILENAME.pkg"
  productbuild --distribution "distribution.xml" --package-path "." --resources "."  "$OUTPUT_BASE_FILENAME.pkg"
  ls -al
  echo "DONE"
fi
